...
def predictBTC(ti, **kwargs):  # Accept 'ti' as a direct argument
    ...
    counter = int(Variable.get("task_counter", default_var=0))
    ...
    if counter%len == 0:
        ...
        modelFile = '/usr/local/airflow/tests/model_rf.pkl'
        print(modelFile)
        ...
        df['predicted'] = y_hat
        df.to_csv('/usr/local/airflow/tests/out_rf.csv', index=False)  
...
def evaluateModel(**kwargs):
    ...
    if counter != 0 and counter%len != 0:
        ...
        file_path = '/usr/local/airflow/tests/out_rf.csv'
        ...
        mape_path = '/usr/local/airflow/tests/mape_rf.txt' #edit mape_rf
    ...
    Variable.set("task_counter", counter) 
    

local_tz = pendulum.timezone("Asia/Bangkok")

with DAG(
    dag_id="test_rf_Predict_BTC",
    schedule_interval="*/5 * * * *", 
    start_date=datetime(2024, 12, 23, 0, 0, 0, tzinfo=local_tz),
    catchup=False,
    tags=["crypto"],
    default_args={
        "owner": "rfnnn",
